...
CUDA_VISIBLE_DEVICES=1 python src/lerobot/scripts/lerobot_train.py \
  ...
  --dataset.root="/home/kwonmc/jiyun/lerobot-VAI/dataset_git/libero_spatial_reproduce" \
  --policy.type="smolvla" \
  ...
  --num_workers=0 \
  --job_name="smolvla_basis_concat_spatial" \
...
CUDA_VISIBLE_DEVICES=1 python src/lerobot/scripts/lerobot_train.py \
  ...
  --wandb.entity="DynamicVLA" \
  --num_workers=0 \
  --job_name="smolvla_basis_concat_object" \
  --policy.visual_cue_mode="basis_concat" \
  --policy.load_vlm_weights=true \
  --policy.freeze_vision_encoder=false \
  --policy.train_expert_only=false